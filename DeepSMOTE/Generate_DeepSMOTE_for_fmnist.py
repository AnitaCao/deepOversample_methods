...
args['n_channel'] = 1 #1    # number of channels in the input data
...
args['dataset'] = 'fMNIST'  #'fmnist' # specify which dataset to use
# File paths
...
class Decoder(nn.Module):
    def __init__(self, args):
        ...
        self.fc = nn.Sequential(
            nn.Linear(self.n_z, self.dim_h * 8 * 7 * 7),  # Adjusted for larger spatial size
            nn.ReLU()
        )
...
dec_x, dec_y = load_fmnist_from_txt(trnimgfile,trnlabfile)
